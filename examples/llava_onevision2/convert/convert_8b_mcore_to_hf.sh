# =============================================================================
# LLaVA-OneVision2 8B – Convert Megatron-Core checkpoint to HuggingFace
# =============================================================================
#
# Usage:
#   bash convert_8b_mcore_to_hf.sh <LOAD> <SAVE> <TP> <PP> [CUSTOM_PIPELINE_LAYERS]
#
# Arguments:
...
#
# Recommended splits for the 8B model (36 LLM layers, 300M ViT on stage-0):
...
#
# Examples:
#   bash convert_8b_mcore_to_hf.sh /src /dst 2 4 0,12,12,12
#   bash convert_8b_mcore_to_hf.sh /src /dst 1 1
# =============================================================================
...
python $CONVERT_CHECKPOINT_PATH/model.py \
    ...
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-8b/qwen3.json \
...
python $CONVERT_CHECKPOINT_PATH/model.py \
    ...
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-8b/vision-model.json \
    --tensor_model_parallel_size=$TP \
...
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision2/adapter.py \
    ...
    --save_platform=huggingface \
    --megatron_path $AIAK_MAGATRON_PATH \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-8b/adapter.json \
    --tensor_model_parallel_size=$TP \
    --pipeline_model_parallel_size=$PP \
    --load_ckpt_path=$LOAD \
...
python $CONVERT_CHECKPOINT_PATH/custom/llava_onevision2/vision_patch.py \
    ...
    --tensor_model_parallel_size=$TP \
    --pipeline_model_parallel_size=$PP \
    --common_config_path=$CONVERT_CHECKPOINT_PATH/config/llava-onevision2-8b/vision-patch.json \
    --load_ckpt_path=$LOAD \
