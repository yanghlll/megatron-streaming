...
# Streaming WebDataset produced by tools/data_preprocess/convert_streaming_to_webdataset.py
DATA_PATH=${DATA_PATH:-"/workspace/dataset/joy_streaming_webdataset"}
TOKENIZER_PATH=${TOKENIZER_PATH:-"/ov2/pretrain_models/preprocessor/preprocessor_llava_onevision1_5"}
CHECKPOINT_PATH=${CHECKPOINT_PATH:-""}

# Streaming decode fps (0 = adaptive by duration: >=160s->1, >=64s->2, else 4).
STREAM_FPS=${STREAM_FPS:-0}
# Per-token loss weights for control tokens (see joy_streaming).
W_SILENCE_FIRST=${W_SILENCE_FIRST:-1.0}
W_SILENCE_REPEATED=${W_SILENCE_REPEATED:-0.4}
W_RESPONSE=${W_RESPONSE:-1.5}
...
DATA_ARGS=(
    ...
    # New control tokens: randomly initialized, learned by full/partial training.
    --additional-special-tokens '</silence>,</response>'
)
...
TRAINING_ARGS=(
    ...
    --w-silence-first "${W_SILENCE_FIRST}"
    --w-silence-repeated "${W_SILENCE_REPEATED}"
    --w-response "${W_RESPONSE}"
    # -------------------------------------
    --seq-length "${SEQ_LEN}"
