...
def normalize_range(range: Tuple[int, int], start):
    """Normalize range based on start."""
    return (range[0] - start, range[1] - start)


class MuonDistMeta:
    """
    Dist meta for muon optimizer
    """
    ...
    def __init__(self, buffer_idx: int, bucket_idx: int, shape: torch.Size,
                 global_range: Tuple[int, int], tp_split_dim: int):
        """Initialize muon dist meta."""
...
class Muon(torch.optim.Optimizer):
    ...
    def enable_distributed_mode(self, global_buffer_sizes, dist_group, tp_group,
        ...
        for bucket_sizes in global_buffer_sizes:
            ...
            for (global_bucket_size, bucket_offset) in bucket_sizes:
                ...
                local_buffer_range = (local_buffer_start,
    ...
    def step(self, origin_params_map=None):
        ...
        if self.distributed_mode:
            ...
            for param, ns_input in ns_inputs.items():
                if origin_params_map is not None:
                    dist_meta = self.dist_metas[origin_params_map[param]]
                else:
            ...
            for p in ns_inputs.keys():
                if origin_params_map is not None:
                    dist_meta = self.dist_metas[origin_params_map[p]]
                else:
                    dist_meta = self.dist_metas[p]
                ns_input_global_buffer = ns_input_global_buffers[
                    dist_meta.buffer_idx][dist_meta.bucket_idx]
                global_range = dist_meta.global_range
                offset = self.global_buffer_sizes[dist_meta.buffer_idx][dist_meta.bucket_idx][1]
                ns_inputs[p] = ns_input_global_buffer[global_range[0] -
                                                      offset: global_range[1] - offset].view(dist_meta.shape)
        ...
        for group in self.param_groups:
            ...
            for p in params:
                ...
                if tp_split_dim != -1:
                    ns_input_shards = [torch.empty_like(
                        ns_input) for _ in range(tp_world_size)]
                ...
                if tp_split_dim != -1:
                    update = update.chunk(
                        tp_world_size, dim=tp_split_dim)[tp_rank]
                ...
                if origin_params_map is not None:
                    p.data.add_(update.cpu(), alpha=-adjusted_lr)
                else:
