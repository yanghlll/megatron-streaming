...
from megatron.training.utils import update_use_dist_ckpt, get_device_arch_version, print_rank_0
...
def validate_args(args, defaults={}):
    ...
    # add aiak for custom pipeline layers check
    if args.custom_pipeline_layers is not None:
        ...
        args.custom_pipeline_layers = pp_splits
    ...
    if args.combined_1f1b:
        ...
        assert (
            args.num_layers_per_virtual_pipeline_stage is not None or 
...
def core_transformer_config_from_args(args, config_class=None):
    ...
    if config_class is None:
        config_class = TransformerConfig if not args.multi_latent_attention else MLATransformerConfig
...
def _add_transformer_engine_args(parser):
    ...
    group.add_argument('--enable-fp8-comm', action='store_true',
                       help='enable fp8 format comm in dispatch',
                       dest='enable_fp8_comm')
...
def _add_network_size_args(parser):
    ...
    group.add_argument('--attention-backend',
    ...
    group.add_argument('--position-embedding-type', type=str, default='learned_absolute',
                        choices=['learned_absolute', 'rope', 'alibi', 'mrope', 'relative', 'none'],
...
def _add_training_args(parser):
    ...
    group.add_argument('--tp-comm-bootstrap-backend', '--tp-comm-overlap-bootstrap-backend',
                       default='nccl', type=str,
                       choices=['nccl', 'mpi', 'gloo'],
                       help='Set the bootstrapping backend of Tensor parallel communications.')
    group.add_argument('--use-cpu-initialization', action='store_true',
                       default=None,
                       help='If set, initialize weights on the CPU. '
                       'This eliminates init differences based on tensor parallelism.')
    ...
    group.add_argument('--check-weight-hash-across-dp-replicas-interval', type=int, default=None,
                       help='Interval to check weight hashes are same across DP replicas. '
                       'If not specified, weight hashes not checked.')
    ...
    group.add_argument('--train-sync-interval', type=int, default=None,
                       help='Training CPU-GPU synchronization interval, '
                       'to ensure that CPU is not running too far ahead of GPU.')
    ...
    # added by aiak for d2d optimizer
    group.add_argument('--d2d-optimizer', type=str, default='disabled', choices=['disabled', 'auto', 'manual'],
                       help='Use d2d in optimizer. It is only used with use-distributed-optimizer.')
    # Todo: not implement.
    group.add_argument('--overlap-d2d-optimizer', type=bool, default=True,
                       help='It is only used for d2d-optimizer, ovelap the comm and compute.')
    group.add_argument('--d2d-max-data-volume', type=int, default=0,
                       help='max data volume of d2d optimizer, in units of G.')
    group.add_argument('--reduced-data-volume-from-pre-stage', type=int, default=0,
                       help='The amount of data reduced from the previous stage, in units of G.')

    #add by aiak for pp layer imbalnce.
    ...
    #add by aiak for recompute layer imbalance.
    group.add_argument('--custom-pipeline-recompute-layers', type=str, default=None,
                       help='add by aiak for recompute layer imbalance.For example 10,11,12,13.'
                       '10 for stage0 recompute layers, 11 for stage1 recompute layers...')

    group.add_argument('--dense-mlp-activation-func-recompute', action='store_true',
                       help='Enable dense model mlp activation_func recompute optimization.')

    group.add_argument('--layernorm-recompute',  action='store_true', help='layernorm recompute')
    group.add_argument('--moe-mlp-activation-func-recompute',  action='store_true', 
                       help='moe mlp activation-func  recompute')
    group.add_argument('--mla-recompute',  action='store_true', help='mla recompute')

    group.add_argument('--enable-mem-monitor',  action='store_true', help='Enable mem monitor')
    group.add_argument('--print-mem-monitor-interval',  type=int, default=100000,
                       help='Interval at which to log memory usage')
    group.add_argument('--mem-monitor-log',  type=str, default=None,
                       help='File path where to store memory monitoring logs')
    group.add_argument('--mem-monitor-force-print-token-threshold',  type=int, default=10000000,
...
def _add_distributed_args(parser):
    ...
    group.add_argument('--wgrad-deferral-limit', type=int, default=0, help='Number of micro-batches for which'
                       ...
                       'is not set')
    group.add_argument("--disable-turn-off-bucketing", action='store_false',
                        dest='enable_turn_off_bucketing',
                        help="whether to bucketing at all pipeline stages")
    ...
    group.add_argument('--suggested-communication-unit-size', type=int, default=400_000_000,
                        ...
                        'this argument guides the size of communication unit size.')
...
def _add_moe_args(parser):
    ...
    group.add_argument('--moe-shared-expert-intermediate-size', type=int, default=None,
                       ...
                       'It should be equal to "num_shared_experts * ffn_size_of_each_shared_expert" '
                       'if there are multiple shared experts. None means no shared expert.')
    ...
    group.add_argument('--moe-grouped-gemm', action='store_true',
                       help='When there are multiple experts per rank, '
                       'launch multiple local GEMM kernels in multiple streams to improve the utilization '
                       'and performance with GroupedLinear in TransformerEngine.')
    # Router arguments
    group.add_argument('--moe-router-load-balancing-type', type=str,
                       ...
                       '"seq_aux_loss" corresponds to the load balancing loss used in DeepSeekV2, '
    ...
    group.add_argument('--moe-router-pre-softmax', action='store_true',
                       help='Enable pre-softmax routing for MoE, which means softmax is before the top-k selection. '
                       'By default, softmax is done after top-k.')
    group.add_argument('--moe-router-num-groups', type=int, default=None,
                       help='Number of groups to divide experts into for group-limited routing.'
                       'When using group-limited routing: '
                       '1) Experts are divided into equal-sized groups, '
                       '2) For each token, a subset of groups are selected based on routing scores '
                       'Two common use cases: '
                       '1) Device-limited routing: Set equal to expert parallel size (EP) to limit each token to '
                       'experts on a subset of devices (See DeepSeek-V2: https://arxiv.org/pdf/2405.04434) '
                       '2) Node-limited routing: Set equal to number of nodes in EP group to limit '
                       'each token to experts on a subset of nodes (See DeepSeek-V3: https://arxiv.org/pdf/2412.19437)')
    ...
    group.add_argument('--moe-router-topk-scaling-factor', type=float, default=None,
                       help='Scaling factor for routing score in top-k selection, '
                       'only works when --moe-router-pre-softmax enabled. Defaults to None, which means no scaling.')
    ...
    group.add_argument('--moe-router-bias-update-rate', type=float, default=1e-3,
                       ...
                       'The expert bias is updated based on the number of assigned tokens to each '
                       'expert in a global batch, where the bias is increased for the experts with less assigned '
                       'tokens and decreased for the experts with more assigned tokens. '
                       'The default value 1e-3 is same as that used in DeepSeekV3.')
    group.add_argument('--moe-use-legacy-grouped-gemm', action='store_true',
                       help='Use legacy GroupedMLP rather than TEGroupedMLP. '
                       'Note: The legacy one will be deprecated soon.')
    group.add_argument('--moe-router-force-load-balancing', action='store_true',
                       help='[Experimental] Force override routing to balance token distribution using random logits'
    ...
    group.add_argument('--moe-token-dispatcher-type', type=str,
                       ...
                       default='allgather',
                       help="The type of token dispatcher to use. The default is 'allgather'. Options are 'allgather', "
                       "'alltoall' and 'alltoall_seq'. We recommend using 'alltoall' when applying expert parallelism. "
    ...
    group.add_argument('--moe-enable-deepep', action='store_true',
                       help='[Experimental] Enable DeepSeek/DeepEP for efficient token dispatching and '
    ...
    group.add_argument('--moe-pad-expert-input-to-capacity', action='store_true',
                       help='Pads the input for each expert to match the expert capacity length, '
    ...
    group.add_argument('--moe-use-upcycling', action='store_true',
                       help='Load a checkpoint of a dense model, convert it into an MoE model, and save the converted '
                       'model to the path specified by --save. Upcycling is implemented on the top of distributed '
    ...
    group.add_argument('--num-nextn-predict-layers', type=int, default=0,
                       help='Number of MTP layers.')
    ...
    group.add_argument('--combined-1f1b-recipe', type=str,
                       choices=['ep_a2a'],
                       default='ep_a2a',
                       help="1) ep_a2a: Hiding expert parallelism's all-to-all communication"
                       "Options are only 'ep_a2a' now.")
...
def _add_mla_args(parser):
    ...
    group.add_argument('--qk-head-dim', type=int, default=128,
                       help="Dimension of the head in the QK projection. "
